...
class TestCalculator(unittest.TestCase):
    ...
    def x_test_add_bracket(self):
        cases: List[TestCase_Bracket] = [
            ...
            TestCase_Bracket(data="3 + 5", expected="(3+5)",
                             desc="외곽 괄호를 잘 붙임"),
            TestCase_Bracket(data="0 + 3", expected="(0+3)",
                             desc="시작하는 0 누락 안 함"),
            TestCase_Bracket(data="3 + 0", expected="(3+0)",
                             desc="끝나는 0 누락 안 함"),
            TestCase_Bracket(data="1 + 0 + 2", expected="(1+0+2)",
                             desc="중간 0 누락 안 함"),
            TestCase_Bracket(data="1/2+3", expected="((1/2)+3)",
                             desc="사칙 연산 순서 보장"),
            TestCase_Bracket(data="2 + 3 * 5", expected="(2+(3*5))",
                             desc="사칙 연산 순서 보장(높은 순위 연산자가 뒤쪽에 있는 경우)"),
            TestCase_Bracket(data="1/(2+3)*5", expected="((1/(2+3))*5)",
                             desc="복잡한 수식"),
            TestCase_Bracket(data="1/(5+3*2)+6", expected="(1/(5+(3*2))+6)",
                             desc="복잡한 내부 수식"),
        ]
